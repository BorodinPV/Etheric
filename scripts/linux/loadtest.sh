...
#
# Usage:
#   ./scripts/linux/loadtest.sh
#   ./scripts/linux/loadtest.sh --vus 20 --duration 1m --scenario refresh
#
...
# Docker bridge gateway (172.17.0.1) is the standard way to reach the host.
BASE_URL="${BASE_URL:-http://172.17.0.1:8080}"
VUS="${VUS:-10}"
...
if curl -sf --connect-timeout 3 "http://localhost:8080/health/live" >/dev/null; then
  ...
  echo "      Start Etheric: ./scripts/linux/dev.sh --no-rate-limit"
fi
...
if ! docker run --rm curlimages/curl:8.5.0 -sf --connect-timeout 3 "$BASE_URL/health/live" >/dev/null; then
  ...
  if [[ "$host_ok" -eq 1 ]]; then
    echo "Etheric runs on localhost but not via Docker bridge."
    echo "Try: ./scripts/linux/loadtest.sh --base-url http://172.17.0.1:8080"
